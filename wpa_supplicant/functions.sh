...
WPA_SUP_BIN="/bin/wpa_supplicant"
...
ifup () {
	...
	if [ -n "$WPA_LOGICAL_IFACE" ]; then
		...
		if /bin/ifquery "$WPA_IFACE" | grep -q '^wpa-roam: ' ; then
			# Force settings over the unconfigured "master" IFACE
			/bin/ifup -v --force "$WPA_IFACE=$WPA_LOGICAL_IFACE"
		else
			/bin/ifup -v "$WPA_IFACE=$WPA_LOGICAL_IFACE"
		fi
		IFUP_RETVAL="$?"
